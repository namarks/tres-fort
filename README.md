...
npx wrangler d1 create tres-forte-db  # first time; paste id into wrangler.jsonc
npx wrangler d1 migrations apply tres-forte-db --remote
...
2. **Name:** anything (e.g. `Tres Forte`).
...
5. Claude discovers the OAuth endpoints and opens a **consent screen**
   titled "Connect tres-forte" with an **Owner passphrase** field.
...
```bash
claude mcp add --transport http --scope user tres-forte \
...
```bash
cd ios
xcodegen generate          # regenerate TresForte.xcodeproj from project.yml
open TresForte.xcodeproj
```

Use the **TresForte** scheme (never the widget-extension scheme). Automatic
